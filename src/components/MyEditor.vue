<template>
  <div>
    <div>
      <button @click="insertTextHandler">插入文字</button>
      <button @click="disableHandler">启用/禁用</button>
      <button @click="addEcharts">插入Echarts</button>
      <button @click="addChartsPic">插入Echarts图片</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    addEcharts() {
      ...
      const node1 = { type: 'echarts', children: [{ text: '' }] }
